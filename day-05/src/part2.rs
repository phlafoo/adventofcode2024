...
pub fn process(input: &str) -> miette::Result<String> {
    ...
    for line in lines {
        ...
        // `last_index` minus the count for that number is equal to the index in the corrected update
        for (i, p) in update.iter().enumerate() {
            if i != last_index - counts[*p] {
                correct = false;
            }
            if counts[*p] == middle_index {
                middle = *p;
            }
        }
        // Only count non-correct updates
        if !correct {
            result += middle;
        }
    }
...
/*
[[97], [75,97,53,61,47], [97,61,29,47,75,53]]
75,29,13

[[97,61,29,47,75,53], [97], [75,97,53,61,47], [97, 75]]
97,13,75,29,47

[97], [97, 75], [97, 47, 75], [47, 75, 61, 97]
75,97,47,61,53
*/
